...
This network would generate data with

\begin{enumerate}
    \item $y$ is sampled from a Bernoulli distribution with parameter $p$
    \item Sample the top $z$ for source 1
    \begin{itemize}
        \item If $y$ is 1, then sample from the $n$-dimensional binary vectors
            of even parity
        \item If $y$ is 0, then sample from the $n$-dimensional binary vectors
            of odd parity
    \end{itemize}
    \item Sample the bottom $z$ for source 2
\end{enumerate}
